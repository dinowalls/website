...
export const Navbar01 = React.forwardRef<HTMLElement, Navbar01Props>(
  (
    ...
    return (
      ...
      >
        <div
          className="container mx-auto flex h-16 max-w-5xl xl:max-w-7xl items-center justify-between gap-4 rounded-2xl border border-white/70 bg-white/82 p-4 px-4 shadow-[0_20px_60px_-35px_rgba(15,23,42,0.45)] backdrop-blur-xl supports-[backdrop-filter]:bg-white/72 md:px-6 [&_*]:no-underline transition-[margin-top] duration-500 ease-in-out dark:border-white/12 dark:bg-slate-950/78 dark:shadow-[0_24px_70px_-42px_rgba(0,0,0,0.78)]"
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            {isMobile && (
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    className="group h-9 w-9 hover:bg-accent hover:text-accent-foreground dark:text-slate-200 dark:hover:bg-white/10 dark:hover:text-white"
                    variant="ghost"
                    size="icon"
                  >
                    <HamburgerIcon />
                ...
                <PopoverContent align="start" className="w-48 border-white/60 bg-white/92 p-2 backdrop-blur-xl dark:border-white/12 dark:bg-slate-950/92">
                    ...
                    {navigationLinks.map((link, index) => (
                      <NavigationMenuItem key={index} className="w-full">
                        <NavigationMenuLink
                          href={link.href}
                          className={cn(
                            "flex w-full items-center rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground cursor-pointer no-underline dark:hover:bg-white/10 dark:hover:text-white",
                            link.active 
                              ? "bg-accent text-accent-foreground dark:bg-white dark:text-slate-950" 
            ...
            <div className="flex items-center gap-6">
              <a 
                href={logoHref}
                className="flex items-center space-x-2 text-slate-950 hover:text-slate-700 transition-colors cursor-pointer dark:text-white dark:hover:text-slate-200"
              >
                <div className="text-2xl text-slate-950 dark:text-slate-950">
                  {logo}
                </div>
                <span className="hidden font-semibold tracking-[-0.04em] text-xl sm:inline-block">dinowalls</span>
              </a>
              {/* Navigation menu */}
              {!isMobile && (
                  ...
                  {navigationLinks.map((link, index) => (
                    <NavigationMenuItem key={index}>
                      <NavigationMenuLink
                        href={link.href}
                        className={cn(
                          "group inline-flex h-9 w-max items-center justify-center rounded-full px-4 py-2 text-sm font-medium transition-colors hover:bg-black/5 focus:bg-black/5 focus:outline-none disabled:pointer-events-none disabled:opacity-50 cursor-pointer no-underline dark:hover:bg-white/10 dark:focus:bg-white/10",
                          link.active 
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="rounded-full text-sm font-medium text-slate-600 hover:bg-black/5 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-white/10 dark:hover:text-white"
              asChild
            ...
            <Button
              size="sm"
              className="h-9 rounded-full bg-slate-950 px-4 text-sm font-medium text-white shadow-sm hover:bg-slate-800 dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200"
              asChild
